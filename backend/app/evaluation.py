...
def _is_processed(by_name: dict) -> bool:
    return all(name in by_name for name in AGENT_ORDER) or all(
        name in by_name for name in LEGACY_AGENT_ORDER
    )
...
def compute_evaluation_metrics(
    ...
    """Compute business KPIs from fully processed leads only."""
    ...
    qualification_rate = qualified_leads / processed_leads if processed_leads else 0.0
    meeting_recommendation_rate = (
        meeting_recommendations / processed_leads if processed_leads else 0.0
    )
    ...
    human_review_rate = human_review_count / processed_leads if processed_leads else 0.0
